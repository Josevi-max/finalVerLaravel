...
class ControlUsersAdmin extends Controller
{
    ...
    public function admin(Request $request){
        ...
        $nextClass= DB::table('date_class')->orderBy("dayClass","asc")->whereIn("StudentId",$ids)->where("dayClass",">", date("Y-m-d H:i"))->take(20)->get();
        $searchs=$request->query("search");
        if($searchs){
            $users=DB::table('users')->where('name', 'like', "%" .$searchs . "%")->orWhere('email', 'like', "%" .$searchs . "%")->paginate(9); 
 
        }elseif(isset($request->myStudents)){
             $users=DB::table('users')->whereIn('id', $ids )->paginate(9); 
        }
         $nUsers = count(User::all());
         return view("components.log.admin-home", compact("users", "nUsers","nextClass"));
    }
}
